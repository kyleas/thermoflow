...
use tf_fluids::{Composition, FluidModel, FrozenPropertySurrogate, ThermoState};
use tf_solver::thermo_policy::{StateCreationResult, ThermoStatePolicy};

/// Sample used to update a surrogate model.
pub struct SurrogateSample {
    pub p: Pressure,
    pub t: f64,
    pub h: f64,
    pub rho: f64,
...
impl TransientFallbackPolicy {
    ...
    pub fn update_surrogate(&mut self, node_id: usize, sample: SurrogateSample) {
        ...
        match fluid.state(tf_fluids::StateInput::PH { p, h }, composition.clone()) {
            ...
            Err(_) => {
                ...
                if let Some(Some(surrogate)) = self.surrogates.get(node_id) {
                    ...
                    // But detect if extrapolation is extreme and apply conservative bounds
                    let t_safe = if !(100.0..=2000.0).contains(&t_est) {
                        // Temperature estimate is extreme - use conservative fallback
                        // For now, clamp to a reasonable range [200K, 500K] for typical fluid conditions
                        eprintln!("[FALLBACK] WARNING: Temperature estimate {:.1} K is extreme, clamping to [200K, 500K]", t_est);
                        t_est.clamp(200.0, 500.0)
                    } else {
                        t_est
                    };

                    // Create a PT state using the safe temperature estimate
                    match ThermoState::from_pt(p, tf_core::units::k(t_safe), composition.clone()) {
                        Ok(state) => {
                            *self.fallback_count.lock().unwrap() += 1;
                            eprintln!("[FALLBACK] Node {} using surrogate (P={:.1} Pa, h={:.1} J/kg, T_est={:.1} K -> {:.1} K clamped)", 
                                     node_id, p.value, h, t_est, t_safe);
                            Ok(StateCreationResult::Fallback(state))
                        }
                        Err(e) => Err(tf_solver::SolverError::InvalidState {
                            what: format!(
                                "Node {} surrogate fallback failed (P={:.1} Pa, T={:.1} K): {}",
                                node_id, p.value, t_safe, e
                            ),
                        }),
                    }
                } else {
...
mod tests {
    ...
    fn policy_succeeds_with_real_fluid() {
        ...
        let p = Pressure::new::<pascal>(100000.0);
        let h = 300000.0;
    ...
    fn policy_uses_fallback_when_available() {
        ...
        policy.update_surrogate(
            1,
            SurrogateSample {
                p: Pressure::new::<pascal>(101325.0),
                t: 300.0,
                h: 300000.0,
                rho: 1.2,
                cp: 1039.0,
