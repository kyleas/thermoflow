...
use tf_core::units::{m, pa, Area, DynVisc, Pressure, Temperature, kgps};
...
impl TransientNetworkModel {
    ...
    pub fn build_timeseries_record(
        ...
        use tf_results::{EdgeValueSnapshot, GlobalValueSnapshot, NodeValueSnapshot, TimeseriesRecord};
        ...
        for (comp_id_str, &comp_idx) in &self.runtime.comp_id_map {
            if let Some((_, mdot)) = solution
                .mass_flows
                .iter()
                .find(|(id, _)| *id == comp_idx)
            {
...
impl TransientModel for TransientNetworkModel {
    ...
    fn rhs(&mut self, t: f64, x: &Self::State) -> SimResult<Self::State> {
        ...
        for (i, (&p, &h)) in solution.pressures.iter().zip(solution.enthalpies.iter()).enumerate() {
            let state = self
                .fluid_model
        ...
        for (comp_id, mdot) in &solution.mass_flows {
            ...
            let component_model = snapshot.components.get(comp_id).ok_or_else(|| SimError::Backend {
    ...
    fn add(&self, a: &Self::State, b: &Self::State) -> Self::State {
        ...
        TransientState { control_volumes: out }
    }
...
fn build_control_volumes(
    ...
    composition: Composition,
) -> Result<(Vec<ControlVolume>, Vec<NodeId>, HashMap<NodeId, usize>, TransientState), String> {
...
fn build_schedule_data(schedules: &[ScheduleDef]) -> ScheduleData {
    ...
    for schedule in schedules {
        for event in &schedule.events {
            match &event.action {
                ActionDef::SetValvePosition { component_id, position } => {
                    data.valve_events
                        .entry(component_id.clone())
                        .or_default()
                        .push((event.time_s, *position));
                }
                ActionDef::SetBoundaryPressure { node_id, pressure_pa } => {
                    data.boundary_pressure_events
                        .entry(node_id.clone())
                        .or_default()
                        .push((event.time_s, *pressure_pa));
                }
                ActionDef::SetBoundaryTemperature { node_id, temperature_k } => {
...
fn build_components_with_schedules(
    ...
    for component in &system.components {
        ...
        let boxed: Box<dyn TwoPortComponent> = match &component.kind {
            ComponentKind::Orifice { cd, area_m2, treat_as_gas } => {
                if *treat_as_gas {
